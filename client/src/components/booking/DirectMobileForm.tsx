...
export function DirectMobileForm({
  ...
  const initialStudioId = selectedStudio || booking?.studioId || studios[0]?.id || null;
  ...
  const [formData, setFormData] = useState<FormBookingData>({
    ...
    title: booking?.title || '',
    description: booking?.description || '',
    studioId: selectedStudio || studios[0]?.id || 0,
    pcrRoomId: booking?.pcrRoomId || null, 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('DirectMobileForm - Final form data with multi-studio support:', submissionData);
    console.log('DirectMobileForm - Studios selected:', submissionData.studioIds);
