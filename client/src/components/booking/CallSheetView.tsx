...
export function fmtDate(d: Date, tz: string) {
  return d.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric", timeZone: tz });
}

interface Props { bookingId: number; }

export function CallSheetView({ bookingId }: Props) {
  ...
  return (
    <div className="bg-white shadow-md print:shadow-none border border-gray-300 print:border-0 overflow-hidden">
      {/* Header */}
      ...
      <div className="bg-[#fef9e7] text-center py-2 px-4 text-sm font-bold border-b border-gray-300 text-[#1e3a5f]">
        {billing}
      </div>
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-[#cfe2c9] text-left text-[#1e3a5f]">
