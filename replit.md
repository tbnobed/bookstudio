...
**Latest Version: 1.5.6** - Adds Decommission workflow to asset edit forms. Both desktop and mobile edit forms include a "Decommission Asset" danger zone requiring a written reason before retiring the equipment. On submit, status is set to `retired` and the reason is stored in the new `decommission_reason` column on the `assets` table. Once decommissioned, the reason displays read-only in the same section.
