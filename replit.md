...
- June 27, 2025: COMPLETELY FIXED midnight time picker across ALL forms - eliminated duplicate "12:00am" React warnings by adding "12:00am (next day)" as final time option, updated time parsing logic to handle "(next day)" format with proper date advancement, preventing confusing same-time bookings and ensuring clear distinction between start-of-day and end-of-day midnight selections across desktop (BookingModal, MobileBookingForm, AlertModal) and mobile HTML time inputs (SimpleMobileForm, DirectMobileForm)
