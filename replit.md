...
- July 7, 2025: REVERTED ALERT SYSTEM TO LEGACY IMPLEMENTATION - restored original alert filtering from bookings table instead of new alerts API due to critical display issues, alerts now use original booking-based filtering for maintenance and IT support bookings with facility-wide scope (studioId: null), maintaining compatibility with existing alert display logic in weekly calendar view
