...
check_database() {
  ...
  while [ $retry_count -lt $MAX_RETRIES ]; do
    local db_ready
    db_ready=$(docker exec -e PGPASSWORD=${POSTGRES_PASSWORD:-postgres} bookstuio-db pg_isready -U "$db_username" -d "$db_name" 2>/dev/null || echo "not_ready")
    
    if [[ "$db_ready" != *"not_ready"* ]]; then
      log "Database is ready and accepting connections."
      
      # Extra verification - try a simple query
      if docker exec -e PGPASSWORD=${POSTGRES_PASSWORD:-postgres} bookstuio-db psql -U "$db_username" -d "$db_name" -c "SELECT 1" >/dev/null 2>&1; then
